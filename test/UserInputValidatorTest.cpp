...
class UserInputValidatorTest : public ::testing::Test {
    ...
    protected:     
        ...
        void validateUsername(ParameterMapPtr paramMap){
            validator.validateUsername(paramMap);
        }
        void validatePassword(ParameterMapPtr paramMap){
            validator.validatePassword(paramMap);
        }
...
TEST_F(UserInputValidatorTest, validatePortNegative){
    ...
    EXPECT_THROW({
        try{
            ...
            // and this tests that it has the correct message (no suitable macro in gtest)
            EXPECT_STREQ( "'port' could not be casted to numeric value! Exception: "
                    "bad lexical cast: source type value could not be interpreted as target", e.what() );
            throw;
        }
    }, std::invalid_argument);
...
TEST_F(UserInputValidatorTest, validateHostnameAndPortNegative){
    ...
    (*paramMap)["hostname"] = "ThisDomainWillNeverEverExistForSureAndILikeWarmCarpetSlippers.com";
        ...
        try{
            ...
            // and this tests that it has the correct message (no suitable macro in gtest)
            EXPECT_STREQ( "Hostname:port could not be resolved! Exception:"
                    " resolve: Host not found (authoritative)", e.what() );
            throw;
        }
    }, std::invalid_argument);
